...
function preencheCampos(TXT_DESCRICAO, id) {
    document.getElementById("txtDescricaoAparelho").value = TXT_DESCRICAO;

    habilitaDesabilitaBotoes(false, id);
}

function cancelarEdicao() {
    document.getElementById("txtDescricaoAparelho").value = "";
    habilitaDesabilitaBotoes(true, 0);
}
...
function cadAltera(id, cadAltera) {
    ...
    if (document.getElementById("txtDescricaoAparelho").value == "")
        document.getElementById("result").textContent =
            "Favor, informe a descrição do aparelho!!!";
    else if (document.getElementById("txtDescricaoAparelho").value.length > 100)
        document.getElementById("result").textContent =
            "Favor, para o campo descrição do aparelho máximo de 100 caracteres permitidos!!!";
    else {
        document.getElementById("result").textContent =
            "Validacao concluida!!!";
        ...
        $.ajax({
            ...
            url: "/setaparelho/" + id + "/" + cadAltera + "/" + idEmpresa,
            data: image,
            processData: false,
            contentType: false,
            async: false,
...
function excluir() {
    ...
    $.ajax({
        type: "GET",
        url: "/delaparelho/" + idSelecionadaParaExcluir,
        processData: false,
        contentType: false,
        async: false,
        beforeSend: function () {},
