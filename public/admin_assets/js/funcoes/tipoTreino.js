...
function cadAltera(id, cadAltera) {
    ...
    if (document.getElementById("txtDescricao").value == "")
        document.getElementById("result").textContent =
            "Favor, informe a descrição do Tipo de Treino!!!";
    else if (document.getElementById("txtDescricao").value.length > 100)
        document.getElementById("result").textContent =
            "Favor, para o campo descrição do Tipo de Treino máximo de 100 caracteres permitidos!!!";
    else {
        document.getElementById("result").textContent =
            "Validacao concluida!!!";
        ...
        $.ajax({
            ...
            url: "/settipotreino/" + id + "/" + cadAltera + "/" + idEmpresa,
            data: image,
            processData: false,
            contentType: false,
            async: false,
...
function preparaExclusao(id) {
    ...
    document.getElementById("result").textContent =
        "Deseja REALMENTE excluir este Tipo de Treino?";
}
...
function excluir() {
    ...
    $.ajax({
        type: "GET",
        url: "/deltipotreino/" + idSelecionadaParaExcluir,
        processData: false,
        contentType: false,
        async: false,
        beforeSend: function () {},
